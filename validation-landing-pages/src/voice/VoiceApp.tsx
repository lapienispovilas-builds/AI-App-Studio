...
import { voiceBrand, voiceFormOptions, voicePages, type VoicePageConfig } from './config'
import './voice.css'
...
function AudioConversationPreview({config:c}:{config:VoicePageConfig}) {
 ...
 return <figure className="v-conversation v-audio-demo">
  <figcaption><div className="v-assistant-brand"><span className="v-avatar"><img src={voiceBrand.symbol} alt="" width={1125} height={991}/></span><span>Revomatix AI voice agent</span></div><strong>{demo.title??c.workflowName}</strong><span>Example AI call</span></figcaption>
  ...
  <div className="v-preview-context"><Phone size={16} aria-hidden="true"/><span>{demo.trigger??c.preview.context}</span></div>
...
function OptionalSelect({id,name,label,options}:{id:string;name:string;label:string;options:readonly string[]}){
...
function PilotForm({ config:c }: { config: VoicePageConfig }) {
 ...
 return <div className="v-form-card"><span className="v-label">LET’S EXPLORE THE FIT</span><h3>{c.formHeading}</h3><p>{c.formIntro}</p><form onSubmit={submit} aria-busy={state==='pending'}><fieldset disabled={state==='pending'||state==='success'}><div className="v-fields"><label htmlFor="pilot-name">Name<input id="pilot-name" name="name" autoComplete="name" required maxLength={120} pattern=".*\S.*" /></label><label htmlFor="pilot-company">Company<input id="pilot-company" name="company" autoComplete="organization" required maxLength={160} pattern=".*\S.*" /></label></div><label htmlFor="pilot-email">Work email<input id="pilot-email" type="email" name="email" autoComplete="email" required maxLength={254}/></label><OptionalSelect id="pilot-volume" name="volume" label={c.form.volumeLabel} options={voiceFormOptions.volume}/><label htmlFor="pilot-current-spend">{c.form.spendLabel} <span>(optional)</span><select id="pilot-current-spend" name="currentSpend" value={currentSpend} onChange={event=>setCurrentSpend(event.target.value)}><option value="">Select an option</option>{voiceFormOptions.spend.map(option=><option key={option}>{option}</option>)}</select></label>{currentSpend==='Yes'&&<label htmlFor="pilot-current-use">What do you currently use? <span>(optional)</span><input id="pilot-current-use" name="currentUse" maxLength={240} placeholder={c.form.currentUsePlaceholder??'e.g. answering service, extra front-desk hours, a software tool'}/></label>}<OptionalSelect id="pilot-channel" name="channelPreference" label={c.form.channelLabel} options={voiceFormOptions.channel}/><label htmlFor="pilot-context">{c.pilot.question} <span>(optional)</span><textarea id="pilot-context" name="context" rows={3} maxLength={2000}/></label><fieldset className="v-checkbox-group"><legend>Do any of these also come up for you? <span>(optional)</span></legend>{c.form.relatedIssues.map(option=><label key={option}><input type="checkbox" name="relatedIssues" value={option}/><span>{option}</span></label>)}</fieldset><label className="v-checkbox"><input type="checkbox" name="anonymizedExamples"/><span>I’d be open to sharing a few anonymized examples</span></label><div className="v-honeypot" aria-hidden="true"><label>Website<input name="website" tabIndex={-1} autoComplete="off"/></label></div><button className="v-button" type="submit">{state==='pending'?'Saving your request…':state==='success'?'Request saved':'Request a pilot conversation'}<ArrowRight size={17} aria-hidden="true"/></button></fieldset><p className="v-preview-note">We’ll use these details to respond to your enquiry. <a href="/privacy">Privacy</a></p>{(state==='error'||state==='success')&&<div ref={status} tabIndex={-1} className="v-form-status" role={state==='error'?'alert':'status'}>{state==='success'?'Your request has been saved. Thank you for your interest in a pilot.':error}</div>}</form></div>
}
function VoiceLandingPage({ config: c }: { config: VoicePageConfig }) {
  ...
  const copy=c.sectionCopy
  return <div className="voice-page" id="top">
    <a className="v-skip" href="#main">Skip to content</a><Header ctaLabel={copy?.ctaLabel}/>
    <main id="main">
      <section className="v-container v-hero"><div className="v-hero-copy"><p className="v-eyebrow"><span className="v-dot" />{c.eyebrow}</p><h1>{c.headline}{' '}<span>{c.headlineAccent}</span></h1><p className="v-lead"><EmphasizedCopy config={c} /></p><div className="v-actions"><PilotLink>{copy?.ctaLabel}</PilotLink><a className="v-text-link" href="#workflow">See the workflow<ArrowDown size={16} aria-hidden="true" /></a></div><p className="v-hero-note">{c.invitation}</p></div><ConversationPreview config={c} /></section>
      {copy&&<section className="v-example-band"><div className="v-container v-example v-section v-copy-split"><div><p className="v-label">{copy.problem.eyebrow}</p><h2>{copy.problem.headline}</h2></div><div><p className="v-lead-small">{copy.problem.body}</p><p className="v-copy-emphasis">{copy.problem.emphasis}</p></div></div></section>}
      <section id="workflow" className="v-workflow-band"><div className="v-container v-section"><div className={`v-section-heading ${copy?'is-single':''}`}><div><p className="v-label">{copy?.workflowEyebrow||'THE WORKFLOW'}</p><h2>{c.workflowTitle}</h2></div>{!copy&&<p>{c.problem}</p>}</div><ol className={`v-process ${c.workflow.length===4?'is-four':''}`}>{c.workflow.map((step,i) => <li key={step.title}><div className="v-step-number">0{i+1}<ArrowRight size={19} aria-hidden="true" /></div><h3>{step.title}</h3>{step.body && <p>{step.body}</p>}</li>)}</ol></div></section>
      {!copy&&<section className="v-example-band"><div className="v-container v-example v-section"><div><p className="v-label">AFTER THE CONVERSATION</p><h2>A useful handoff.<br />A clear owner.</h2><p className="v-lead-small">{c.preview.owner}</p></div><div className="v-handoff"><div className="v-handoff-heading"><MessageSquare size={20} aria-hidden="true" /><strong>{c.preview.status}</strong></div><dl><div><dt>What they needed</dt><dd>{c.preview.need}</dd></div><div><dt>What was recorded</dt><dd>{c.preview.recorded}</dd></div><div><dt>Who acts next</dt><dd>{c.preview.owner}</dd></div></dl></div></div></section>}
      <section className="v-value-band"><div className="v-container v-section v-value"><p className="v-label">{copy?.valueEyebrow||'WHAT WE’LL LEARN'}</p><h2>{copy?.valueHeadline||<>A practical pilot.<br/>A clearer picture of the value.</>}</h2><div className={`v-value-list ${copy?'is-cards':''}`}>{c.values.map((value,i) => <article key={value.title}><span>0{i+1}</span><h3>{value.title}</h3>{value.body && <p>{value.body}</p>}</article>)}</div></div></section>
      {copy&&<section className="v-differentiation-band"><div className="v-container v-section v-differentiation"><div><p className="v-label">{copy.differentiation.eyebrow}</p><h2>{copy.differentiation.headline}</h2></div><div><p>{copy.differentiation.body}</p><strong>{copy.differentiation.emphasis}</strong></div></div></section>}
      <section id="pilot" className="v-container v-pilot-band"><div className="v-pilot"><div><h2>{c.pilot.title}</h2><p className="v-lead-small">{c.pilot.description}</p></div><div><ul>{c.pilot.scope.map(s => <li key={s}><Check size={18} />{s}</li>)}</ul><p className="v-compatibility">We’ll review your software and messaging setup during the pilot discussion.</p><PilotLink>{copy?.ctaLabel}</PilotLink></div></div></section>
      <section id="faq" className="v-faq-band"><div className="v-container v-section v-faq"><div><p className="v-label">A FEW GOOD QUESTIONS</p><h2>Before we talk.</h2></div><div>{c.faqs.map(f => <details key={f.question}><summary>{f.question}<span aria-hidden="true">+</span></summary><p>{f.answer}</p></details>)}</div></div></section>
      <section id="contact" className="v-closing-band"><div className="v-container v-closing v-section"><div><p className="v-label">LET’S EXPLORE A PILOT</p><h2>{c.finalTitle}</h2><p className="v-lead-small">One focused workflow. Calls, {c.channel === 'SMS' ? 'texts' : 'emails'} and a clear next step for your team.</p></div><PilotForm config={c} /></div></section>
    </main>
    <footer className="v-container v-footer"><a className="v-brand" href="#top"><img className="v-brand-image" src={voiceBrand.logo} alt="Revomatix" width={1575} height={437} /></a><span>{c.inbound ? 'Voice-first intake. Human next steps.' : 'Voice-first follow-up. Human next steps.'}</span><span>© {new Date().getFullYear()} Revomatix</span><a href="/privacy">Privacy</a>{privacyContact&&<a href={`mailto:${privacyContact}`}>Contact</a>}<AnalyticsChoice/></footer>
  </div>
}
export function App() {
